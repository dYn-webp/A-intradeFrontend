...
import { 
  ...
  Coffee, Bot, Users, Quote, Target, Heart, Globe, ChevronRight
} from "lucide-react";
...
const LiveTickerItem = ({ id, initialPrice, initialChange, isIDR }: TickerItemProps) => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      const movement = (price * (Math.random() * 0.002)); 
...
const MarketTicker = () => {
  ...
  return (
    <div className="w-full bg-[#0a0c10] border-b border-white/5 overflow-hidden flex items-center h-8 relative">
      <motion.div 
        animate={{ x: ["0%", "-50%"] }} 
...
export default function LandingPage() {
  ...
  return (
    <div className="bg-[#0a0c10] min-h-screen text-white font-sans selection:bg-[#D4AF37] selection:text-black overflow-x-hidden">
      
      {/* 1. Navbar */}
      <div className="fixed top-0 w-full z-50">
        <MarketTicker />
        <nav className="border-b border-white/5 bg-[#0a0c10]/80 backdrop-blur-xl">
          <div className="container mx-auto px-6 h-16 flex items-center justify-between">
            <div className="text-xl font-black tracking-tighter flex items-center">
              A&apos;INTRADE<span className="text-[#D4AF37] text-2xl leading-[0]">.</span>
            </div>
            ...
            <div className="flex items-center space-x-4">
              ...
              <Link href="/dashboard" className="px-4 py-2 bg-[#D4AF37] text-black text-xs font-bold rounded-md hover:opacity-90 transition shadow-[0_0_15px_rgba(212,175,55,0.3)]">
                Launch App
              </Link>
            </div>
          </div>
      ...
      <section className="relative pt-32 pb-20 min-h-[85vh] flex items-center">
        <div className="absolute top-1/4 left-1/4 w-[400px] h-[400px] rounded-full blur-[120px] opacity-10 bg-[#D4AF37] pointer-events-none"></div>
        <div className="container mx-auto px-6 relative z-10 grid lg:grid-cols-2 gap-12 items-center">
          <div className="space-y-6">
            <div className="inline-flex items-center space-x-2 px-3 py-1 rounded-full border border-[#D4AF37]/30 bg-[#D4AF37]/10 text-[#D4AF37] text-xs font-bold">
          ...
          <div className="bg-[#151922]/90 backdrop-blur-xl border border-white/10 rounded-3xl p-6 relative shadow-2xl">
            <div className="flex bg-black/50 rounded-full p-1 border border-white/5 w-fit mb-6">
              <button onClick={() => setActiveTab('trading')} className={`px-4 py-1.5 rounded-full text-xs font-bold transition-all ${activeTab === 'trading' ? 'bg-[#D4AF37] text-black' : 'text-gray-400 hover:text-white'}`}>Micro Prop-Firm</button>
              <button onClick={() => setActiveTab('invest')} className={`px-4 py-1.5 rounded-full text-xs font-bold transition-all ${activeTab === 'invest' ? 'bg-blue-500 text-white' : 'text-gray-400 hover:text-white'}`}>Dip-Funding Loan</button>
            </div>
              ...
              {activeTab === 'trading' ? (
                <motion.div key="trading" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-5">
                  <div className="flex justify-between items-center border-b border-white/5 pb-4">
                    <div className="flex items-center space-x-2"><BarChart3 className="text-[#D4AF37] w-5 h-5" /><h3 className="font-bold text-sm">Simulasi (Biaya: {formatMoney(15000)})</h3></div>
                  ...
                  <div className="p-3 bg-[#D4AF37]/10 rounded-xl border border-[#D4AF37]/20 text-xs font-bold flex justify-between"><span className="text-[#D4AF37]">Bagi Hasil</span><span className="text-white">Hingga 80% ke rekening Anda</span></div>
                </motion.div>
              ) : (
                <motion.div key="invest" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-5">
                  <div className="flex justify-between items-center border-b border-white/5 pb-4">
                    <div className="flex items-center space-x-2"><HandCoins className="text-blue-400 w-5 h-5" /><h3 className="font-bold text-sm">Margin Calculator</h3></div>
                  </div>
                  <div>
                    ...
                    <input type="range" min="15000" max="5000000" step="5000" value={walletBalance} onChange={(e) => setWalletBalance(Number(e.target.value))} className="w-full h-1.5 bg-black rounded-lg appearance-none cursor-pointer accent-blue-500" />
                  </div>
                  <div className="p-4 bg-blue-500/10 rounded-xl border border-blue-500/30 flex justify-between items-center">
                    <div><div className="text-xs font-bold text-gray-400">Total Daya Beli (Leverage 5x)</div><div className="text-[10px] text-blue-400">Disubsidi A&apos;INTRADE: +{formatMoney(buyingPower - walletBalance)}</div></div>
      ...
      {/* 3. Features Highlight (EKOSISTEM YANG SEMPAT HILANG) */}
      <section id="ecosystem" className="py-20 border-y border-white/5 bg-black/50">
        <div className="container mx-auto px-6">
          <div className="text-center mb-12">
            <h2 className="text-2xl font-bold mb-2">Desain Khusus Untuk <span className="text-[#D4AF37]">Retail Trader Lokal</span></h2>
            <p className="text-gray-400 text-sm">Akses ekosistem finansial tanpa batas minimum yang memberatkan kantong.</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              { title: "Mulai dari Rp 15 Ribu", desc: "Biaya pendaftaran challenge prop-firm seharga secangkir kopi. Sempurna untuk melatih psikologi trading pemula.", icon: CheckCircle2 },
              { title: "Pinjaman Skala Mikro", desc: "Tidak perlu jaminan aset. Gunakan performa trading Anda untuk mendapatkan fasilitas margin mulai dari puluhan ribu rupiah.", icon: Wallet },
              { title: "Payout Instan (QRIS / Bank)", desc: "Pencairan profit skala kecil diproses secara instan langsung ke rekening bank lokal atau e-Wallet Anda.", icon: Zap }
            ].map((f, i) => (
              <div key={i} className="p-6 bg-[#151922] border border-white/5 rounded-2xl hover:border-[#D4AF37]/30 transition">
      ...
      {/* 4. Visi Misi */}
      <section id="vision" className="py-20 border-b border-white/5 bg-[#0a0c10]">
        <div className="container mx-auto px-6 max-w-4xl text-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-left">
            <div className="bg-[#151922] border border-white/5 p-6 rounded-2xl">
      ...
      <section id="founder" className="py-24 container mx-auto px-6">
        <div className="bg-gradient-to-br from-[#151922] to-black border border-[#D4AF37]/20 rounded-3xl p-8 md:p-12 relative overflow-hidden">
          <div className="absolute top-0 right-0 w-64 h-64 bg-[#D4AF37]/10 blur-[80px]"></div>
          ...
          <div className="flex flex-col md:flex-row gap-12 relative z-10 items-center">
            <div className="w-full md:w-1/3">
               <div className="aspect-[4/5] rounded-3xl bg-black border border-white/10 p-6 flex flex-col justify-end relative group overflow-hidden">
                  ...
                  <div className="relative z-20">
                    ...
                    <p className="text-sm text-[#D4AF37] mb-4">Founder & System Architect</p>
                    <div className="flex space-x-2">
                      ...
                      <span className="px-2 py-1 bg-[#D4AF37]/10 text-[#D4AF37] rounded text-[10px] flex items-center"><Cpu className="w-3 h-3 mr-1" /> Mandor AI</span>
                    </div>
                  </div>
               </div>
            </div>
            ...
            <div className="flex-1 space-y-6">
              ...
              <div className="space-y-4 text-gray-400 text-sm md:text-base leading-relaxed">
                <p>
                  Beberapa waktu lalu, profesi saya adalah seorang <strong>pramusaji (waiter)</strong>. Bekerja keras seharian, namun uang di dompet hanya cukup untuk bertahan hidup. Boro-boro untuk *trading* jutaan rupiah.
                </p>
                <p>
                  Rasa frustrasi itu mendorong saya belajar. Karena saya tidak punya tim programmer, saya bertindak sebagai <strong>Mandor AI</strong>—memerintahkan *AI agents* untuk merancang arsitektur, dan membangun ekosistem <em>A&apos;INTRADE</em> dari nol.
                </p>
                <p>
      ...
      {/* 6. Real Community (TESTIMONIAL YANG SEMPAT HILANG) */}
      <section id="community" className="py-24 container mx-auto px-6 border-t border-white/5">
         <div className="text-center mb-16">
          <h2 className="text-3xl font-bold mb-4">Mereka yang Sudah <span className="text-[#D4AF37]">Membuktikan</span></h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {[
            ...
            { name: "Rina", role: "Karyawan Shift", quote: "Dulu sering MC karena maksain depo gede. Di A'INTRADE, mental dilatih pelan-pelan. Fitur pinjaman margin-nya juga ngebantu banget pas XAUUSD lagi crash." },
            ...
            <div key={i} className="bg-[#151922] border border-white/5 p-6 rounded-2xl relative">
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <div className="text-[10px] text-[#D4AF37]">{t.role}</div>
                </div>
              </div>
            </div>
          ))}
      ...
      <footer className="py-8 border-t border-white/5 bg-black text-center">
        <div className="container mx-auto px-6">
          <div className="text-lg font-black mb-2">A&apos;INTRADE<span className="text-[#D4AF37]">.</span></div>
